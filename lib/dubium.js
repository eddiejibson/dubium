...
const valuesInRange = (to, from = null) => {
    ...
    while (time <= from) {
        range.push(new Date(time).getTime());
    ...
    if (match) {
        ...
        for (var i = 0; i < months.length; i++) {
            if (months[i].substr(0, 3).toLowerCase() == str.substr(match.index, 3).toLowerCase()) {
                if (match2) {
                    str = str.replace(months[i], "MMMM")
                } else {
                    str = str.replace(months[i].substr(0, 3), "MMM")
                }
